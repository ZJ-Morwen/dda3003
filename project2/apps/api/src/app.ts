...
export async function buildApp() {
  ...
  app.get("/api/dashboard/snapshot", async (request) => {
    ...
    return getDashboardSnapshot(query.startDate, query.endDate);
  });

  app.get("/api/voyages/emissions-scatter", async (request) => {
    const query = request.query as { startDate?: string; endDate?: string };
    return getScatter(query.startDate, query.endDate);
  });

  app.get("/api/voyages/:voyageId/route", async (request, reply) => {
    const params = request.params as { voyageId: string };
    const query = request.query as { startDate?: string; endDate?: string; ts?: string };
    const payload = await getVoyageRoute(params.voyageId, query.startDate, query.endDate, query.ts);
    if (!payload) {
  ...
  app.get("/api/voyages/:voyageId/emission-series", async (request, reply) => {
    const params = request.params as { voyageId: string };
    const query = request.query as { startDate?: string; endDate?: string };
    const payload = await getVoyageEmissionSeries(params.voyageId, query.startDate, query.endDate);
    if (!payload) {
  ...
  app.get("/api/voyages/:voyageId/metrics", async (request, reply) => {
    const params = request.params as { voyageId: string };
    const query = request.query as { startDate?: string; endDate?: string };
    const payload = await getVoyageMetrics(params.voyageId, query.startDate, query.endDate);
    if (!payload) {
  ...
  app.get("/api/port-flows", async (request) => {
    const query = request.query as { startDate?: string; endDate?: string };
    return getPortFlows(query.startDate, query.endDate);
  });

  app.get("/api/port-flows/:source/:target/voyages", async (request) => {
    const params = request.params as { source: string; target: string };
    const query = request.query as { startDate?: string; endDate?: string };
    return getPortFlowVoyages(params.source, params.target, query.startDate, query.endDate);
  });

  app.get("/api/environment/weights", async (request) => {
    const query = request.query as { startDate?: string; endDate?: string };
    return getEnvironmentWeights(query.startDate, query.endDate);
  });
